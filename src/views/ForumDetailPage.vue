<template>
  <div class="forum-detail-page">
    <div class="container">
      ...
      <div v-else-if="!post" class="error-box">Post tidak dijumpai.</div>
      ...
      <div v-else>
        ...
        <div class="comment-section">
          ...
          <div class="comment-input-box">
            <h3>Sertai Perbincangan</h3>
            <textarea 
              v-model="mainCommentText" 
              rows="3" 
              :placeholder="auth.currentUser ? 'Tulis komen anda...' : 'Sila login untuk komen.'"
              ...
              <div class="comment-item">
                ...
                <div class="comment-body-wrapper">
                  ...
                  <div class="comment-actions">
                    <button @click="toggleReplyBox(comment.id)">↩️ Balas</button>
                  </div>

                  <div v-if="activeReplyId === comment.id" class="reply-input">
                    <textarea v-model="replyText" rows="2" placeholder="Balas komen ini..."></textarea>
                    <div class="reply-btns">
                      <button class="btn-cancel-small" @click="activeReplyId = null">Batal</button>
                      <button class="btn-submit-small" @click="submitComment(comment.id)">Hantar</button>
                    </div>
                  </div>
                </div>
              </div>
...
const allComments = ref<any[]>([]); // Simpan SEMUA komen (ibu & anak)
const loading = ref(true);

// Input Models
const mainCommentText = ref('');
const replyText = ref('');
const activeReplyId = ref<string | null>(null); // ID komen yang sedang dibalas
const comments = ref<any[]>([]);
// Helper: Tapis Komen Ibu (Tiada Parent)
const rootComments = computed(() => {
  return allComments.value.filter(c => !c.parentId);
});

// Helper: Cari Anak Komen (Berdasarkan Parent ID)
...
onMounted(async () => {
  ...
  // 2. Listener Komen (Sub-collection)
  const commentsRef = collection(db, "forum_posts", postId, "comments");
  const q = query(commentsRef, orderBy("createdAt", "asc")); // Susun ikut masa (lama atas, baru bawah)

  onSnapshot(q, (snapshot) => {
    allComments.value = snapshot.docs.map(doc => ({
      id: doc.id,
      ...doc.data()
    }));
  });
...
// Logic Hantar Komen (Ibu atau Anak)
const submitComment = async (parentId: string | null) => {
  if (!auth.currentUser) return;

  // Tentukan teks mana nak guna
  ...
  try {
    await addDoc(collection(db, "forum_posts", postId, "comments"), {
      ...
      votes: 0,
      parentId: parentId // Kunci utama (Null = Ibu, String = Anak)
    });

    // Update jumlah komen post utama
...
const voteComment = async (comment: any, val: number) => {
  if (!auth.currentUser) return alert("Login untuk undi.");
  
  // Update UI serta merta (Optimistic UI)
...
const toggleReplyBox = (id: string) => {
  if (!auth.currentUser) return alert("Login untuk balas.");
