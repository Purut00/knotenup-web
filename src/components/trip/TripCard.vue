<template>
  <div class="trip-card">
    <div class="status-badge" :class="trip.status">
      {{ trip.status === 'open' ? 'Open' : trip.status === 'full' ? 'Penuh' : 'Tutup' }}
    </div>
    ...
    <div class="card-content">
      ...
      <div class="progress-section">
        <div class="progress-label">
          ...
          <span class="spots-left" v-if="trip.maxSlots - trip.currentSlots <= 5">
            🔥 Tinggal {{ trip.maxSlots - trip.currentSlots }}!
          </span>
        </div>
      ...
      <button class="btn-join">Lihat Detail</button>
    </div>
  </div>
</template>
...
defineProps<{
  trip: any // Kita guna 'any' dulu untuk cepat, nanti boleh define interface
}>();
</script>
...
/* Progress Bar */
.progress-section { margin-top: auto; } /* Tolak ke bawah */
.progress-label {
  display: flex;
  justify-content: space-between;
  font-size: 0.75rem;
