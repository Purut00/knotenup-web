<template>
  <!-- Card Container -->
  <div 
    class="relative flex flex-col overflow-hidden rounded-2xl border border-white/10 bg-white/5 backdrop-blur-md text-white shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-[#6c63ff] hover:shadow-xl cursor-pointer group h-full"
    ...
    <div class="flex flex-1 flex-col justify-between p-4" :class="{ 'p-3': isCompact }">
      ...
      <div>
        <div class="flex items-center justify-between mb-1">
          <span class="text-[10px] font-semibold uppercase tracking-wider text-orange-400">{{ translateCategory(trip.category) }}</span>
          <div v-if="!isCompact" class="flex items-center gap-1 text-xs font-medium text-yellow-400">
            <i class="fas fa-star"></i> {{ trip.rating || '5.0' }}
          </div>
        </div>

        <h3 class="font-medium text-white leading-snug group-hover:text-[#6c63ff] transition-colors"
            :class="isCompact ? 'text-sm line-clamp-2' : 'text-lg line-clamp-2 mb-2'">
          {{ trip.title }}
        </h3>

        <!-- Info Row -->
        <div class="flex gap-4 text-xs text-slate-400" :class="{ 'flex-col gap-1 mt-0': isCompact, 'mt-1': !isCompact }">
           <div class="flex items-center gap-1.5">
              <i class="far fa-calendar-alt text-[#a78bfa]"></i>
              <span>{{ formattedDate }}</span>
           </div>
           <div class="flex items-center gap-1.5">
              <i class="far fa-clock text-[#a78bfa]"></i>
      ...
      <!-- Footer / Progress -->
      <div class="mt-3 border-t border-white/10 pt-3" :class="{ 'border-none mt-0 pt-0': isCompact }">
        ...
        <div v-if="!isCompact" class="mb-3">
           <div class="flex justify-between text-[10px] font-medium text-slate-400 mb-1">
             ...
             <span v-if="trip.maxSlots - trip.currentSlots <= 5" class="text-red-400 animate-pulse">
               🔥 {{ trip.maxSlots - trip.currentSlots }} left!
             </span>
           </div>
           <div class="h-1.5 w-full rounded-full bg-white/10 overflow-hidden">
             <div class="h-full bg-gradient-to-r from-[#6c63ff] to-orange-400 rounded-full" 
        ...
        <div class="flex items-center justify-between" :class="{ 'justify-end': isCompact }">
           <div v-if="!isCompact" class="flex items-center gap-2">
             <img :src="trip.organizerImage || 'https://i.pravatar.cc/150'" class="h-6 w-6 rounded-full border border-white/20 object-cover">
             <span class="text-xs text-slate-300 truncate max-w-[100px]">{{ trip.organizerName }}</span>
           </div>
           
           <div class="flex items-baseline gap-0.5 text-white">
             <span class="text-[10px] text-slate-400">RM</span>
